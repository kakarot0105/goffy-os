...
| Mac file enumeration creep | `mac.files.list` is disabled until explicit approved roots are configured, lists bounded metadata only, hides dotfiles by default, rejects traversal, and reports symlinks without following targets | Android route/UX controls and future file-content tools need separate review |
