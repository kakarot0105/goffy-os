...
| Clipboard exfiltration | `mac.clipboard.read` is disabled until an optional macOS provider is installed and `GOFFY_MAC_CLIPBOARD_READ_ENABLED=true`; it returns bounded plaintext only on explicit authenticated invocation, performs no background polling, never reads content during health checks, rejects plaintext containing `file://`, and exposes no binary formats or file URLs | Android route, visible approval UX, and richer clipboard type policy need separate review |
