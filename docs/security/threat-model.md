...
| Mac app-launch authority creep | `mac.apps.open` is disabled unless `GOFFY_MAC_APP_ALLOWLIST` and `GOFFY_MAC_APP_OPEN_ENABLED=true` are both configured; Android routes only bounded `open <app> on my Mac` commands, rejects chained connector words, and has a one-time visible approval scaffold; the Hub maps display names only to fixed allowlisted bundle identifiers, runs `/usr/bin/open -b` with `shell=False`, verifies the app is running, exposes no file-open argument, and blocks CONFIRM tools on the current SAFE-only WebSocket and MCP transports | Hub-validated approval artifacts and richer app selection UX |
