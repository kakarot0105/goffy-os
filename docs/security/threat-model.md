...
| Background camera access through flashlight | Use `setTorchMode` without opening a camera stream or image capture; QR scanning is the only current `CAMERA` permission user, and it is foreground-only | Physical OEM validation |
| Background camera access through QR transfer/read | CameraX is mounted only inside the visible scanner panel, bound to lifecycle, guarded against late bind after close, and shut down on dismiss, capture, or Activity stop. Generic QR reads persist only a bounded/redacted summary, not raw sensitive payloads or images | Physical Moto G camera-indicator, QR read, and lifecycle validation |
