...
| Mac process metadata creep | `mac.processes.list` registers only on macOS Hub hosts, fails closed off macOS, and returns bounded process-name basenames, PID, status, RSS memory, and optional start time only; it rejects path-like names on Android, omits command lines, executable paths, environment variables, open files, network data, current working directories, and user names, uses `psutil` without shell execution, does not store process names in terminal audit, and does not read process names aloud | Future process control, filtering, or per-app actions need separate permission and approval review |
| Mac file enumeration creep | `mac.files.list` and `mac.files.largest` are disabled until explicit approved roots are configured; Android sends bounded default-root arguments only after exact capability/schema discovery; the Hub returns metadata only, hides dotfiles by default, rejects traversal, reports or skips symlinks without following targets, and never exposes absolute root paths | Richer root/path UX and future file-content tools need separate review |
...
| Clipboard exfiltration | `mac.clipboard.read` is disabled until an optional macOS provider is installed and `GOFFY_MAC_CLIPBOARD_READ_ENABLED=true`; Android routes only exact explicit Mac clipboard commands, sends no arguments, displays bounded text in the visible timeline, does not speak clipboard text aloud, performs no background polling, never reads content during health checks, rejects plaintext containing `file://`, and exposes no binary formats or file URLs | Visible approval UX and richer clipboard type policy need separate review |
| Mac app-launch authority creep | `mac.apps.open` is disabled unless `GOFFY_MAC_APP_ALLOWLIST` and `GOFFY_MAC_APP_OPEN_ENABLED=true` are both configured; Android routes only bounded `open <app> on my Mac` commands, rejects chained connector words, and requires a one-time visible approval; the Hub has `goffy.approval.v1` request/response groundwork bound to authenticated principal, task ID, tool name, canonical argument SHA-256, issue time, and expiry, but WebSocket and MCP execution remain fail-closed until Android responses include device-bound approval proof; the tool maps display names only to fixed allowlisted bundle identifiers, runs `/usr/bin/open -b` with `shell=False`, verifies the app is running, and exposes no file-open argument | Device-bound approval proof, richer app selection UX, and durable cross-process approval storage if multi-process Hub deployment is introduced |
...
| Background camera access through flashlight | Use `setTorchMode` without opening a camera stream or image capture; QR/OCR scanning is the only current `CAMERA` permission user, and it is foreground-only | Physical OEM validation |
| Background camera access through QR/OCR transfer/read | CameraX is mounted only inside visible scanner panels, bound to lifecycle, guarded against late bind after close, and shut down on dismiss, capture, or Activity stop. Generic QR and OCR reads persist only bounded/redacted summaries, not raw sensitive payloads or images | Physical Moto G camera-indicator, QR/OCR read, and lifecycle validation |
