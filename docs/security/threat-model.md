...
| Git authority creep | `git.status` is disabled until explicit approved repo roots are configured; clients choose only repo index and bounded count; the Hub runs fixed `git status --porcelain=v2` with `shell=False`, no credential prompt, no network operation, and metadata-only output | Android route/UX plus future diff, test, commit, push, and GitHub tools need separate review |
