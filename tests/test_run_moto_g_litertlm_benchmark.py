...
def test_execute_runs_fixed_instrumentation_and_pulls_result(
    ...
    def runner(command: Sequence[str], cwd: Path, timeout: int) -> CommandResult:
        ...
        if "instrument" in normalized:
            assert DEVICE_RESULT_PATH in normalized
            assert "benchmark prompt" in normalized
            assert "timeoutMillis" in normalized
            assert "300000" in normalized
    ...
    assert any("instrument" in command for command in seen)
